...
def send(sender, data, timestamp):
    ...
    if sender.type == "RENFORCE ECG":
        ...
        sender.outlet.push_chunk(
            data,
            timestamp=timestamp + _start_time_lsl + timestamp_offset,
            pushthrough=True,
        )
...
def send_callback(sensor_data):
    ...
    if isinstance(sensor_data, ECGData):
        ...
        sensor_df["outlet"].push_sample(
            [data_value],
            timestamp=timestamp + _start_time_lsl,
            pushthrough=True,
        )
