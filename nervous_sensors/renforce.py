...
async def sensor_disconnected_callback(self : RenforceSensor):
    ...
    global _run_flag
    for sensor in _sensors :
        if sensor.is_connected() and _run_flag == True:
...
async def main(file_path, enable_lsl, enable_gui, sensors_list):
    ...
    for sensor_name in sensors_list:
        _sensors.append(RenforceSensor(sensor_name,
...
if __name__ == "__main__":
    ...
    parser.add_argument('-l', '--lsl', action='store_true', help="Send sensor data on LSL outlets")
    parser.add_argument('-s', '--sensors', help="ECGxxx,EDAxxx... : Give the name of the sensors you want"
