...
class ModernStyle:
    ...
    def configure_styles(cls, style: ttk.Style):
        ...
        style.map('Modern.TButton',
                 background=[('active', cls.ACCENT_PURPLE),
                           ('pressed', '#0056a3')])
        ...
        style.map('Modern.TEntry',
                fieldbackground=[('focus', cls.BG_ACCENT)],
        ...
        style.configure('Modern.TNotebook.Tab',
                    ...
                    font=('Segoe UI', 10, 'bold'),
                    borderwidth=0,
                    width=20)  # Add fixed width to prevent size changes
        ...
        style.map('Modern.TNotebook.Tab',
                ...
                padding=[])  # Empty list means no state-specific padding changes
...
class SensorStatusFrame(ttk.Frame):
    ...
    def setup_ui(self):
        ...
        title_label = ttk.Label(title_frame, text="🔌 Sensor Status", 
        ...
        ttk.Label(headers_frame, text="SENSOR", 
                 font=('Segoe UI', 9, 'bold'),
                 foreground=ModernStyle.TEXT_MUTED,
                 background=ModernStyle.BG_SECONDARY).grid(row=0, column=0, sticky='w', padx=10)
        
        ttk.Label(headers_frame, text="CONNECTION", 
                 font=('Segoe UI', 9, 'bold'),
                 foreground=ModernStyle.TEXT_MUTED,
                 background=ModernStyle.BG_SECONDARY).grid(row=0, column=1, sticky='w', padx=10)
        
        ttk.Label(headers_frame, text="STATUS", 
                 font=('Segoe UI', 9, 'bold'),
                 foreground=ModernStyle.TEXT_MUTED,
                 background=ModernStyle.BG_SECONDARY).grid(row=0, column=2, sticky='w', padx=10)
    ...
    def update_sensor_status(self, sensor_name: str, connected: bool, notifications: bool):
        ...
        if sensor_name not in self.sensor_labels:
            ...
            name_label = ttk.Label(row_frame, text=f"📡 {sensor_name}",
                                  font=('Segoe UI', 10, 'bold'),
            ...
            conn_indicator = ttk.Label(conn_frame, text="●", font=('Segoe UI', 12),
                                      background=ModernStyle.BG_SECONDARY)
            conn_indicator.pack(side='left', padx=(0, 5))
            
            conn_label = ttk.Label(conn_frame, font=('Segoe UI', 10),
                                  background=ModernStyle.BG_SECONDARY)
            ...
            notif_indicator = ttk.Label(notif_frame, text="●", font=('Segoe UI', 12),
                                       background=ModernStyle.BG_SECONDARY)
            notif_indicator.pack(side='left', padx=(0, 5))
            
            notif_label = ttk.Label(notif_frame, font=('Segoe UI', 10),
                                   background=ModernStyle.BG_SECONDARY)
        ...
        if connected:
            self.sensor_labels[sensor_name]['connection_indicator'].config(
        ...
        if notifications:
            self.sensor_labels[sensor_name]['notification_indicator'].config(
                foreground=ModernStyle.ACCENT_GREEN)
        ...
        self.status_indicator.config(
            foreground=ModernStyle.ACCENT_GREEN if any_connected else ModernStyle.ACCENT_RED)
...
class NervousGUI:
    ...
    def setup_ui(self):
        ...
        title_label = ttk.Label(header_frame, text="Nervous Control Panel", 
                               style='Title.TLabel')
        title_label.pack(side='left')
    ...
    def setup_config_tab(self, parent):
        ...
        # Sensors section
        sensors_frame = ttk.LabelFrame(parent, text="🔧 Sensors Configuration", 
                                      style='Modern.TLabelframe', padding=15)
        sensors_frame.pack(fill='x', pady=(0, 20))
        
        # Sensor input with modern styling
        input_label = ttk.Label(sensors_frame, text="Add New Sensor", 
        ...
        self.sensor_entry = ttk.Entry(sensor_input_frame, style='Modern.TEntry', 
        ...
        add_btn = ttk.Button(sensor_input_frame, text="➕ Add", 
                           style='Modern.TButton', command=self.add_sensor)
        add_btn.pack(side='left')
        
        # Sensor list with modern styling
        list_label = ttk.Label(sensors_frame, text="Configured Sensors", 
                              style='Modern.TLabel', font=('Segoe UI', 11, 'bold'))
        ...
        scrollbar = ttk.Scrollbar(listbox_frame, orient='vertical', 
                                 command=self.sensor_listbox.yview)
        ...
        # Remove button
        ttk.Button(sensors_frame, text="🗑️ Remove Selected", 
                  style='Danger.TButton', command=self.remove_sensor).pack(pady=(0, 0))
        ...
        ttk.Checkbutton(gui_container, text=" Enable GUI Dashboard", 
                       variable=self.gui_var, style='Modern.TCheckbutton').pack(side='left')
        
        self.dashboard_btn = ttk.Button(gui_container, text="🚀 Open Dashboard", 
                                       style='Modern.TButton', command=self.open_dashboard, 
                                       state='disabled')
        self.dashboard_btn.pack(side='right')
        
        # Other options
        ttk.Checkbutton(output_frame, text=" Enable LSL Streaming", 
                       variable=self.lsl_var, style='Modern.TCheckbutton').pack(anchor='w', pady=5)
        
        ttk.Checkbutton(output_frame, text=" Enable File Recording", 
                       variable=self.file_var, style='Modern.TCheckbutton').pack(anchor='w', pady=5)
        ...
        ttk.Button(folder_frame, text="Browse", style='Modern.TButton', 
        ...
        ttk.Label(parallel_frame, text="🔗 Parallel Connections:", 
                 style='Modern.TLabel').pack(side='left')
        
        ttk.Spinbox(parallel_frame, from_=1, to=10, textvariable=self.parallel_var, 
                   style='Modern.TSpinbox', width=8).pack(side='left', padx=(10, 0))
    
    def setup_status_tab(self, parent):
        """Setup the status monitoring tab with modern styling"""
        # System status
        system_frame = ttk.LabelFrame(parent, text="⚙️ System Status", 
                                     style='Modern.TLabelframe', padding=15)
        ...
        self.status_label = ttk.Label(status_container, text="● System Stopped", 
        ...
        sensor_status_frame = ttk.LabelFrame(parent, text="📡 Sensor Monitoring", 
                                           style='Modern.TLabelframe', padding=15)
    ...
    def setup_control_buttons(self, parent):
        ...
        self.start_button = ttk.Button(button_container, text="🚀 Start Framework", 
                                      command=self.start_framework, style='Success.TButton')
        self.start_button.pack(side='left', padx=(0, 15))
        
        self.stop_button = ttk.Button(button_container, text="⏹️ Stop Framework", 
